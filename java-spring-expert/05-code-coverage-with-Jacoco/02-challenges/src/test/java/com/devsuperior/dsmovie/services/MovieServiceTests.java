...
public class MovieServiceTests {
	...
	void setUp() throws Exception{
		...
		List<MovieEntity> list = List.of(movie);
		page = new PageImpl<>(list);
		
		Mockito.when(repository.searchByTitle(ArgumentMatchers.anyString(), ArgumentMatchers.any(Pageable.class))).thenReturn(page);
	...
	public void findAllShouldReturnPagedMovieDTO() {
		...
		String title = " O auto da compadecida";
        ...
        Assertions.assertEquals(result.getContent().get(0).getTitle(), page.getContent().get(0).getTitle());
        Assertions.assertEquals(result.getContent().get(0).getScore(), page.getContent().get(0).getScore());
        Assertions.assertEquals(result.getContent().get(0).getImage(), page.getContent().get(0).getImage());
	}
	...
	public void insertShouldReturnMovieDTO() {
		...
		Assertions.assertEquals(movieDTO.getScore(), movie.getScore());
		Assertions.assertEquals(movieDTO.getCount(), movie.getCount());
		Assertions.assertEquals(movieDTO.getImage(), movie.getImage());
	}
	
	@Test
	public void updateShouldReturnMovieDTOWhenIdExists() {

		movieDTO = service.update(existingId, movieDTO);
